...
reshape2::melt(pred_pop_data) |> mutate(obs = rep(na.omit(validation_data), each =1000)) |>
  mutate(ratio = value / obs) |> select(variable, ratio) |> group_by(variable) |> 
  summarise(median=median(ratio), 
            lwr=quantile(ratio, probs=0.025),
            upr=quantile(ratio, probs=0.975)) |>
  mutate(organs = organs, study=study, dose=dose) |> 
  unite("Lable", study:dose)
```
